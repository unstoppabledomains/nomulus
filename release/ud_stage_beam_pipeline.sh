...
# Use public Maven repos (project bucket may not be populated yet)
maven_gcs_prefix="https://repo.maven.apache.org/maven2"
plugins_gcs_prefix="https://plugins.gradle.org/m2"
nom_build_dir="$(dirname $0)/.."
${nom_build_dir}/nom_build :core:"${uberjar_task}" \
    --mavenUrl="${maven_gcs_prefix}" \
    --pluginsUrl="${plugins_gcs_prefix}"
...
while (( "$#" > 0 )); do
  ...
  # Build the gcloud command with optional Cloud Build service account and logs bucket
  gcloud_cmd="gcloud dataflow flex-template build \
  ...
  if [ -n "${SERVICE_ACCOUNT_EMAIL}" ]; then
    gcloud_cmd="${gcloud_cmd} --cloud-build-service-account=projects/${dev_project}/serviceAccounts/${SERVICE_ACCOUNT_EMAIL}"
  fi
  
  # Add GCS log directory if LOGS_BUCKET is set (for Cloud Build logs)
  if [ -n "${LOGS_BUCKET}" ]; then
    gcloud_cmd="${gcloud_cmd} --gcs-log-dir=gs://${LOGS_BUCKET}/dataflow-build-logs"
  fi
